...
fileglobal liveQuery, queryResults, mad, cit, st, arbico, arbicoinv, invno, mtliveQuery, fbliveQuery, sdsliveQuery, mtqueryResults, fbqueryResults, sdsqueryResults, findinwalkin, walkingroup, walkinname, chargeit, giftamount, giftused, orderdisplay
getseedscat=1
gettreescat=1
getbulbscat=1
mtliveQuery=""
...
//Rachel commented out the below specifically for their copy to use with the Taxes as only com orders are needed
/*
...
*/˛/.Initializeˇ˛.NewRecordˇif Paid=0 and Total>0 and PurchaseOrder="" and TransactionType notcontains "donation" and TransactionType notcontains "transfer" and TransactionType notcontains "owes"
...
if «C#»>0
    ...
    if info("selected")<info("records")
        WalkInPurchases=WalkInPurchases+totalitarian
        field WalkInPurchases
        copycell
        pastecell
        call discountfill
        selectall
...
call ".addtoordernew"
˛/.newpoplistˇ˛.newwalkinfindˇlocal findinwalkin
...
if clipboard() contains "yes"
    ...
    stop
endif˛/.deleterecordˇ˛WalkinMonthlyˇlocal seedsselected, raya
raya=0
select Transaction = int(Transaction)
seedsselected=info("selected")
;selectwithin Date ≥ month1st((month1st(today())-1)) and Date < month1st(today()) 
...
arrayselectedbuild raya,¶,"", "OGS/MT"+¬+str(Transaction)+¬+?(TaxExempt contains "Y","N","Y")+¬+"ME"+¬+"Clinton"+¬+pattern(val("04927"),"#####")+¬+¬+¬+¬+¬+¬+¬+¬+str(Adjtotal)+¬+
str(?(TaxTotal>0,Adjtotal,0))+¬+str(SalesTax)+¬+datepattern(val(Date),"Month YYYY")+¬+¬+¬+¬+¬+¬+¬+str(Total)
    ;clipboard()=raya
    ;message raya
    ;stop
 openfile "WayfairSalesTax"
 openfile "+@raya"   
;call "monthlytotals/t"˛/WalkinMonthlyˇ˛sourcegetˇ
local Dictionary, ProcedureList
//this saves your procedures into a variable
//step one
saveallprocedures "", Dictionary
clipboard()=Dictionary˛/sourcegetˇ